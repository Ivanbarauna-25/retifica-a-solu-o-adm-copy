...
export default function RelatorioContasReceberFiltersModal({ isOpen, onClose, clientes }) {
  ...
  const loadPlanosContas = async () => {
    try {
      ...
      const planosReceita = (planos || []).filter(p => p.tipo === 'receita' && p.ativa !== false);
      setPlanosContas(planosReceita);
    } catch (error) {
  ...
  const handleGerar = () => {
    ...
    const url = `${createPageUrl('RelatorioContasReceber')}?${params.toString()}`;
        ...
        <DialogHeader className="modern-modal-header bg-gradient-to-r from-slate-800 to-slate-700 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-200 mt-1">Contas a Receber</p>
            </div>
          </div>
        </DialogHeader>
          ...
          <Card className="border-2 border-slate-200 shadow-sm">
            ...
            <CardContent className="p-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-slate-900 font-semibold mb-2 block">Cliente</Label>
                  <Select 
