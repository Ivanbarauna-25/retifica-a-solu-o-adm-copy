...
export default function EspelhoPontoCompleto({ isOpen, funcionario, mesReferencia, onClose }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-w-6xl w-[98vw] sm:w-full modern-modal overflow-y-auto max-h-[95vh]">
        <style>{`
          @media print {
            html, body {
              margin: 0;
        ...
        <div className="print-page">
          ...
          {/* Header - tela */}
          <div className="no-print bg-slate-800 text-white px-3 md:px-4 py-2 md:py-3 rounded-lg mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-sm md:text-lg font-bold">Espelho de Ponto - {mesReferencia}</h2>
              <p className="text-slate-300 text-[10px] md:text-xs">{funcionarioNome}</p>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={handlePrint} className="gap-2 bg-white text-slate-800 hover:bg-slate-100 text-xs md:text-sm h-8 md:h-9">
                <Printer className="w-3.5 h-3.5 md:w-4 md:h-4" />
                Imprimir
              </Button>
              <Button variant="outline" onClick={onClose} className="gap-2 bg-white text-slate-800 hover:bg-slate-100 text-xs md:text-sm h-8 md:h-9">
