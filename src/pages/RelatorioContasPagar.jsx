...
export default function RelatorioContasPagarPage() {
  ...
  const filterFornecedorId = urlParams.get('fornecedor_id') || '';
  ...
  const fetchData = async () => {
    setIsLoading(true);
    const [contasData, fornecedoresData, planosData, configData] = await Promise.all([
    ...
    if (filterFornecedorId) {
      contasFiltradas = contasFiltradas.filter(c => c.fornecedor_id === filterFornecedorId);
    }
    ...
    if (ordenacao === 'fornecedor') {
      contasFiltradas.sort((a, b) => {
        const fornA = fornecedoresData.find(f => f.id === a.fornecedor_id)?.nome || '';
        const fornB = fornecedoresData.find(f => f.id === b.fornecedor_id)?.nome || '';
  ...
  const totalPago = contas.reduce((total, c) => total + (c.valor_pago || 0), 0);
      ...
      <div className="report-container" translate="no">
        <div className="header-section">
          <div className="company-info">
            <div className="company-name">{config?.nome_empresa || "Relatório de Contas a Pagar"}</div>
        ...
        <div className="report-title">Relatório de Contas a Pagar</div>

        <div className="filters-line" translate="no">
          <div className="flex gap-4 flex-wrap">
            {filterFornecedorId && <span>Fornecedor: {getFornecedorNome(filterFornecedorId)}</span>}
            ...
            <table translate="no">
                ...
                {contas.length === 0 ? (
                  ...
                  contas.map((conta) => (
                    <tr key={conta.id}>
                      ...
                      <td>{getFornecedorNome(conta.fornecedor_id)}</td>
                      ...
                      <td className="text-right">{formatCurrency(conta.valor_pago || 0)}</td>
                      <td>
              ...
              <tfoot>
                <tr>
                  ...
                  <td className="text-right font-bold text-white">{formatCurrency(totalPago)}</td>
                  <td></td>
                </tr>
              </tfoot>
            </table>
