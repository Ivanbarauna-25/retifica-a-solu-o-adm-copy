          ...
          <div className="max-w-[1800px] mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg md:text-2xl lg:text-3xl font-bold">Cotações de EPI</h1>
                  <p className="text-slate-300 text-xs md:text-sm">Gestão de cotações</p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mb-3 md:mb-6">
              ...
              <CardContent className="p-3 md:p-5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xl md:text-3xl font-bold text-slate-900">{cotacoes.length}</div>
                  </div>
            ...
            <Card className="border-l-4 border-l-yellow-500 shadow-sm hover:shadow-md transition-shadow">
              <CardContent className="p-3 md:p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10px] md:text-sm font-medium text-slate-600 mb-1">Pendentes</p>
            ...
            <Card className="border-l-4 border-l-green-500 shadow-sm hover:shadow-md transition-shadow">
              <CardContent className="p-3 md:p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10px] md:text-sm font-medium text-slate-600 mb-1">Aprovadas</p>
                    <div className="text-xl md:text-3xl font-bold text-green-600">
                      {cotacoes.filter(c => c.status === 'aprovada').length}
                    </div>
                  </div>
                  <CheckCircle className="w-6 h-6 md:w-8 md:h-8 text-green-300" />
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-l-4 border-l-red-500 shadow-sm hover:shadow-md transition-shadow">
              <CardContent className="p-3 md:p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10px] md:text-sm font-medium text-slate-600 mb-1">Rejeitadas</p>
                    <div className="text-xl md:text-3xl font-bold text-red-600">
                      {cotacoes.filter(c => c.status === 'rejeitada').length}
                    </div>
                  </div>
                  <XCircle className="w-6 h-6 md:w-8 md:h-8 text-red-300" />
                </div>
              </CardContent>
            </Card>
          </div>
            ...
            <Card className="mb-3 md:mb-4 border-slate-200 shadow-sm">
              <CardContent className="p-3 md:p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  <div>
                    <label className="text-xs font-medium text-gray-600 mb-1 block">Busca</label>
                      ...
                      <TableRow key={cotacao.id} className="hover:bg-slate-50 bg-white">
                        ...
                        <TableCell className="text-right text-black bg-white">
                          <div className="flex justify-end gap-1">
                            <Button variant="ghost" size="icon" onClick={() => handleView(cotacao)} title="Visualizar" className="h-8 w-8 md:h-9 md:w-9 hover:bg-blue-50 text-blue-600">
                              <Eye className="w-4 h-4" />
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => handlePrint(cotacao)} title="Imprimir" className="h-8 w-8 md:h-9 md:w-9 hover:bg-slate-50 text-slate-600 hidden md:flex">
                              <Printer className="w-4 h-4" />
                            </Button>
                            {canEditCotacao && cotacao.status === 'pendente' && (
                              <>
                                <Button variant="ghost" size="icon" onClick={() => handleAprovar(cotacao)} title="Aprovar" className="text-green-600 hover:bg-green-50 h-8 w-8 md:h-9 md:w-9 hidden lg:flex">
                                  <CheckCircle className="w-4 h-4" />
                                </Button>
                                <Button variant="ghost" size="icon" onClick={() => {
                                  const motivo = prompt('Motivo da rejeição:');
                                  if (motivo) handleRejeitar(cotacao, motivo);
                                }} title="Rejeitar" className="text-red-600 hover:bg-red-50 h-8 w-8 md:h-9 md:w-9 hidden xl:flex">
