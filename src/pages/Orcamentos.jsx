...
import { Plus, Printer, Eye, Filter, Upload, FileText, BarChart3, Search, Pencil, MoreVertical, Trash2, Loader2, MessageCircle } from "lucide-react";
...
export default function OrcamentosPage() {
  ...
  const filteredOrcamentos = useMemo(() => {
    ...
    if (quickSearchNumber) {
      const q = quickSearchNumber.toLowerCase();
      filtered = filtered.filter((o) => String(o.numero_orcamento || "").toLowerCase().includes(q));
      return filtered;
    }
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter((o) => {
        const clienteNome = (mapCliente[o.contato_id] || mapCliente[o.cliente_id] || "").toLowerCase();
        const vendedorNome = (mapFuncionario[o.vendedor_id] || "").toLowerCase();
        return clienteNome.includes(s) || vendedorNome.includes(s);
      });
    }

    if (filterStatus !== "todos") filtered = filtered.filter((o) => o.status === filterStatus);
    if (filterVendedor !== "todos") filtered = filtered.filter((o) => o.vendedor_id === filterVendedor);

    if (filterDataInicio) filtered = filtered.filter((o) => o.data_orcamento >= filterDataInicio);
    if (filterDataFim) filtered = filtered.filter((o) => o.data_orcamento <= filterDataFim);
      ...
      <div className="min-h-screen bg-slate-50">
              ...
              <div className="relative">
                ...
                {quickSearchNumber ? (
                  <button
                    ...
                    className="absolute right-2 md:right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 text-xs"
                  >
                    ✕
                  </button>
                ) : null}
                    ...
                    filteredOrcamentos.map((orcamento) => {
                      ...
                      return (
                        <TableRow key={orcamento.id} className="hover:bg-slate-50">
                          <TableCell className="font-medium text-blue-600 py-2 md:py-3 text-xs md:text-sm">
                          ...
                          <TableCell className="text-slate-900 px-6 py-1 text-xs md:py-3 md:text-sm max-w-[100px] md:max-w-none truncate">
                            {cliente?.nome || "-"}
                          </TableCell>

                          <TableCell className="text-slate-900 py-2 md:py-3 text-xs md:text-sm hidden md:table-cell">
                            {veiculoTexto}
                          </TableCell>

                          <TableCell className="text-slate-900 py-2 md:py-3 text-xs md:text-sm hidden lg:table-cell">
                            {vendedor?.nome || "-"}
                          </TableCell>
                          ...
                          <TableCell className="pr-1 pl-2 md:py-3 hidden sm:table-cell">
                            <Badge className="bg-slate-50 text-slate-950 px-1.5 py-0.5 text-xs font-semibold rounded-full border">
                              {statusLabels[orcamento.status] || "-"}
                            </Badge>
                          </TableCell>

                          <TableCell
                            className="text-slate-600 py-2 md:py-3 max-w-[200px] truncate text-xs md:text-sm hidden xl:table-cell"
                            title={orcamento.observacoes}
                          >
                            {orcamento.observacoes || "-"}
                          </TableCell>

                          <TableCell className="pt-1 pr-3 pb-1 pl-3 md:py-3">
                            <div className="flex items-center justify-center gap-0.5 md:gap-2">
                              <Button
                                ...
                                title="Visualizar"
                                className="text-blue-600 hover:bg-blue-50 h-7 w-7 md:h-8 md:w-8 p-0"
                              >
                              ...
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <Button type="button" variant="ghost" size="sm" className="hover:bg-slate-100 text-slate-600 h-7 w-7 md:h-8 md:w-8 p-0">
                                ...
                                <DropdownMenuContent align="end">
                                  ...
                                  <DropdownMenuItem onClick={() => handleDelete(orcamento)} className="text-red-600">
        ...
        <Dialog open={confirmDeleteOpen} onOpenChange={setConfirmDeleteOpen}>
          <DialogContent className="bg-white">
            <DialogHeader>
              <DialogTitle className="text-neutral-900 text-xl font-bold">Tem certeza que deseja excluir este orçamento?</DialogTitle>
            ...
            <DialogFooter className="gap-3">
              <Button type="button" variant="outline" onClick={() => setConfirmDeleteOpen(false)} className="bg-white border-2 border-neutral-400 text-neutral-900 hover:bg-neutral-100 font-bold">
                Cancelar
              </Button>
              <Button type="button" onClick={confirmarExclusao} className="bg-red-600 hover:bg-red-700 text-white font-bold">
